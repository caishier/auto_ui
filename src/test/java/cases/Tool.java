...
public class Tool {
	...
	public void init(String browserType,String driverPath) {
		...
		if ("ie".equalsIgnoreCase(browserType)) {//如果是IE
			...
			getURL(URL);
			getWaitElement(By.id("username")).sendKeys("caixuan");//登录小强系统
			getWaitElement(By.id("password")).sendKeys("123456");
			getWaitElement(By.id("submit")).click();
	...
	public static WebElement getWaitElement(By locator) {
		...
		try {
			...
			logger.error("轮询查找50S，元素定位失败，超时");
		}
		return null;
	}
